...
Script to sort CLC workbech output DNALunge and extract exons 
...
#-----------------
files_in_dir = os.listdir("DNALunge")
csv_files = list(filter(lambda f: f.endswith(".csv"), files_in_dir))

csv_data_lst = []  

for i in range(len(csv_files)):
    with open("DNALunge/"+csv_files[i]) as csv_file:
...
exon_table_OncoHS_filename = "exontableDNALunge.xlsx"
