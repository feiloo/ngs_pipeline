...
Script to sort CLC workbech output BRCAness and extract exons 
...
import os
import pandas as pd
import debuggingOncoHS_DNALunge_BRCANESS.OncoHS_python.functions_processing_variantlists as pv
...
#-----------------
files_in_dir = os.listdir("BRCAness")
csv_files = list(filter(lambda f: f.endswith(".csv"), files_in_dir))

csv_data_lst = []  

for i in range(len(csv_files)):
    with open("BRCAness/"+csv_files[i]) as csv_file:
...
exon_table_OncoHS_filename = "exontableBRCAness.xlsx"
